...
object CoreConfigContextBuilder {
    ...
    private fun resolveFallbackOutbounds(
        ...
        return resolvedOutbounds
            ...
            .filter {
                it !in AppConfig.BUILTIN_OUTBOUND_TAGS
                    && resolvedOutbounds.none { outbound -> outbound.tag == it }
            }
            .distinct()
            .mapNotNull { tag ->
                SettingsManager.getServerViaRemarks(tag)
                    ?.takeUnless {
                        it.configType == EConfigType.CUSTOM || it.configType == EConfigType.POLICYGROUP
                    }
