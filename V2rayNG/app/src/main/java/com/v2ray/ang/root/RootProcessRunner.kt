...
/**
 * Runs one root command with a hard deadline that covers output draining, not just
 * [Process.waitFor]. Neither an inherited stdout nor an oversized output volume can
 * extend the deadline: a detached helper that keeps the pipe open after `su` exits
 * would otherwise block a plain [java.io.Reader.readText] forever.
 */
